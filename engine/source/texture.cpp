...
static constexpr int GetTextureFormat(int channels)
{
	switch (channels)
	{
...
Texture::Texture(std::uint8_t const* data, Ivec2 size, int channels)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size[0], size[1], 0, GetTextureFormat(channels), GL_UNSIGNED_BYTE, data);
...
Texture::Texture(std::string const& path, int channels)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size[0], size[1], 0,
		GetTextureFormat(channels == 0 || ch < channels ? ch : channels), GL_UNSIGNED_BYTE, data);
